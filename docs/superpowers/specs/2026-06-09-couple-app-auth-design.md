...
두 사람(한 커플)이 사용하는 **커플 앱**. PWA(설치형 웹)로 배포하며 Android·iOS 모두 지원.
...
- 1차로 계정 1개(`kcs___chang`)만 시드. 2번째 계정과 `Couple` 연결은 후속 마일스톤.
- 아이디 + 비밀번호 로그인, 로그아웃, 세션 유지, 보호 라우트.

### 범위 밖 (후속 마일스톤)
2번째 계정, 커플 페어링, D-day, 기념일, 갤러리, 푸시 알림.
...
- 기존 `Diary` 모델은 **제거**한다(커플 앱으로 피벗하여 미사용).
